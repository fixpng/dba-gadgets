...
# 定义基本目录和日期变量
MONGORESTORE="/usr/local/mongodb/bin/mongorestore"
MONGOIMPORT="/usr/local/mongodb/bin/mongoimport"
...
if [ -f "$USERS_FILE" ]; then
    ...
    $MONGOIMPORT --host "$HOST" --port "$PORT" --username "$USER" --password "$PASS" --authenticationDatabase "$AUTH_DB" --db admin --collection system.users --file "$USERS_FILE"
else
...
for BACKUP_FILE in $BACKUP_FILES; do
    ...
    gunzip -c "$BACKUP_FILE" | $MONGORESTORE --host "$HOST" --port "$PORT" --username "$USER" --password "$PASS" --authenticationDatabase "$AUTH_DB" --archive --gzip
done

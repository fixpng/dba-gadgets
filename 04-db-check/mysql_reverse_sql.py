...
'''
mysql_reverse_sql 是一个用于解析和转换 MySQL 二进制日志（binlog）的工具。它可以将二进制日志文件中记录的数据库更改操作（如插入、更新、删除）转换为反向的 SQL 语句，以便进行数据恢复。其运行模式需二进制日志设置为 ROW 格式。
https://github.com/hcymysql/reverse_sql/

# mysql8
pip3 install --upgrade pytz pymysql mysql-replication tqdm -i http://mirrors.aliyun.com/pypi/simple/ --trusted-host mirrors.aliyun.com/pypi/simple/
# mysql57
pip3 install --upgrade pytz pymysql mysql-replication==0.45.1 tqdm -i http://mirrors.aliyun.com/pypi/simple/ --trusted-host mirrors.aliyun.com/pypi/simple/

# 获取帮助
python3 ./mysql_reverse_sql.py --help
...
# 执行完毕后会在当前目录生成回滚文件
python3 ./mysql_reverse_sql.py -ot t2 -op delete -H 127.0.0.1 -P 3306 -u root -p aaaa12345* -d my_test2 --binlog-file 947cbc66f564-bin.000001 --start-time "2025-02-20 21:40:00" --end-time "2025-02-20 21:43:00"
'''
            ...
            if isinstance(binlogevent, WriteRowsEvent):
                ...
                if only_operation and only_operation != 'delete':
                    ...
                    sql = "DELETE FROM `{}` WHERE {};".format(
                        "`{}`.`{}`".format(database_name, binlogevent.table) if database_name else "`{}`".format(binlogevent.table),
                        ' AND '.join(["`{}`={}".format(k, 
                            "'{}'".format(json.dumps(v, ensure_ascii=False) if isinstance(v, (dict, list)) else v)
                            if isinstance(v, (str, datetime.datetime, datetime.date, dict, list))
                            else 'NULL' if v is None
...
Example usage:
    ...
    parser.add_argument('-v', '--version', action='version', version='reverse_sql工具版本号: 2.1.2，更新日期：2024-08-09')
